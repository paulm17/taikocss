 ...
 */
function serializeObject(obj, parentSelector, output) {
    let declarations = '';
    for (const key in obj) {
        ...
        if (typeof value === 'object' && !Array.isArray(value)) {
            ...
            const cssValue = formatValue(cssProperty, value);
 ...
 */
export function _serializeCSS(className, styles) {
...
// ---------------------------------------------------------------------------
const cache = new Set();
function injectStyle(hash, cssText) {
    if (cache.has(hash))
        return;
    cache.add(hash);
 ...
 * ```
 */
export function runtimeCss(styles) {
 ...
 * ```
 */
export function runtimeGlobalCss(styles) {
    let css = '';
    for (const selector in styles) {
        ...
        if (typeof value === 'object' && !Array.isArray(value)) {
            const nestedBlocks = [];
            const declarations = serializeObject(value, selector, nestedBlocks);
            if (declarations) {
                css += selector + '{' + declarations + '}';
            }
            if (nestedBlocks.length > 0) {
